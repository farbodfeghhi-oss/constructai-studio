...
import {
  ...
  BookOpen, FlaskConical, ShieldCheck, ChevronDown, ChevronRight,
} from "lucide-react";
...
const CHECKLIST_SECTIONS = [
  {
    ...
    items: [
      "3D-Modell vollständig und fehlerfrei",
      "Alle Bauteile korrekt benannt",
      "Baugruppen-Struktur logisch aufgebaut",
      "Kollisionsprüfung durchgeführt",
      "Bewegungssimulation getestet (falls relevant)",
    ],
  },
  {
    titel: "Technische Zeichnungen",
    items: [
      "Alle Maße und Toleranzen eingetragen",
      "Oberflächenangaben vollständig",
      "Passungen korrekt spezifiziert",
      "Schriftfeld ausgefüllt (Datum, Version, Bearbeiter)",
      "Schnittansichten wo nötig",
    ],
  },
  {
    titel: "Normprüfung & Material",
    items: [
      "Alle Normteile korrekt referenziert (DIN/ISO)",
      "Werkstoffauswahl dokumentiert und begründet",
      "Werkstoffzertifikate angefordert (falls nötig)",
      "Oberflächenbehandlung spezifiziert",
    ],
  },
  {
    titel: "Fertigung & Montage",
    items: [
      "Fertigungsverfahren festgelegt",
      "Biegeradien prüfen (Blechfertigung)",
      "Montagereihenfolge dokumentiert",
      "Anzugsdrehmomente festgelegt",
      "Sonderwerkzeuge identifiziert",
    ],
  },
  {
    titel: "Freigabe",
    items: [
      "Stückliste vollständig und geprüft",
      "Kostenabschätzung erstellt",
      "Technische Review durchgeführt",
      "Freigabe durch Projektleiter",
      "Dokumentation an Fertigung übergeben",
    ],
  },
];
    ...
    <div className="max-w-5xl space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Technische Unterlagen, Stücklisten, Normen-Referenz und Release-Checkliste.
        </p>
      </div>
      ...
      <Tabs defaultValue="stueckliste">
        ...
        <TabsContent value="normen" className="space-y-4 mt-4">
          <div className="relative">
            ...
            <Input
              placeholder="Norm suchen (z.B. DIN 933, Schraube, Blech)…"
              value={normenSuche}
              onChange={(e) => setNormenSuche(e.target.value)}
              className="pl-9"
            />
          </div>
          <Card>
        ...
        <TabsContent value="material" className="space-y-4 mt-4">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input
              placeholder="Material suchen (z.B. S235, Edelstahl, Aluminium)…"
              value={matSuche}
          ...
          <div className="grid gap-4">
              ...
              <Card key={m.name}>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-sm">
                    <div>
                      <p className="text-muted-foreground text-xs">Streckgrenze</p>
                      <p className="font-mono font-medium">{m.reZug}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground text-xs">Zugfestigkeit</p>
                      <p className="font-mono font-medium">{m.rm}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground text-xs">Dichte</p>
                      <p className="font-mono font-medium">{m.dichte}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground text-xs">E-Modul</p>
                      <p className="font-mono font-medium">{m.eModul}</p>
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                      <p className="text-muted-foreground text-xs">Einsatzgebiet</p>
                      <p className="text-sm">{m.einsatz}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            {filteredMat.length === 0 && (
              <p className="text-center text-muted-foreground py-8">Kein Material gefunden.</p>
            )}
          </div>
        </TabsContent>
